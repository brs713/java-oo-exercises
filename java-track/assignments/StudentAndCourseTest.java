...
public class StudentAndCourseTest extends TestCase {
	...
	@Test
	public static void testCreateLegacy() {
		for(int i = 0; i < 100; i++) {
			double a =  (Math.random() * 5000);
			double b =  (Math.random() * 5000);
...
//			System.out.println("ma = " + s.getStudentID());
	...
	public static void testGetAllCourses() {
		int courseCount = Course.getAllCourses().size();
		int limit = (int)(Math.random() * 1000);
		for (int i = 0; i < limit; ++i) {
			Course c = new Course("" + limit, limit, limit);
		}
		assertEquals("getAllCourses is messed up!", Course.getAllCourses().size(), (courseCount + limit));
	}
